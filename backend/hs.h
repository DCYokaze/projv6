...
public:
  ...
  int initARFF(string outName);//,int bytes, unsigned int min, unsigned int max);
  int updateARFF(string inName,string outName,int bytes,int shift, long min, long arrLen);
  int * longHexToArrInterval(int bytes,int shift, string & longLine, long min, long arrLen);
  int longHexToArrInterval_nged(int * arrIn, int bytes, int shift, string fileName, long min, long arrLen);

//private:
  std::string longToHex( long l,int bytes);
  string featureSelectionToFile(int bytes, int shift, int h_start=0, int h_doForNSegment=0);
  ...
  int readFromNgedArr(int * arr, string fileName, int ngram, long min, long arrLen);
  int readFromNgedArrBool(bool * arrBool, string fileName, int ngram, long min, long arrLen);

  int IGBinaryToFile(int, long, long, long);
  std::vector<attrValue> IGBinaryEval(int, long, long, double, bool **, double *);
  
  int TfidfToFile(int bytes, long inStart, long inEnd, long batchSize);
  std::vector<attrValue> TfidfEval(int, long, long, double, int **, double *, long *, int *);
