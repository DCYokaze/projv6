...
int HS::longHexToArray(int *ArrResult, int bytes, int shift, string longLine) {
  // int* attrValueHex = new int[commonLib::powInt(256,bytes)];//to keep all
  // value.
...
{                      // cout<<"call init arff"<<endl;
  ...
  if (oFile.is_open()) {
    ...
    // for(std::vector<std::string>::iterator it = vectorOfFile.begin(); it !=
    // vectorOfFile.end(); ++it)
    for (int i = 0; i < vectorOfFile.size(); i++) {
      string fileName = vectorOfFile[i];
...
int HS::updateARFF(string inName, string outName, int bytes, int shift,
  ...
  string aLargerBuff = "";
  {
    ...
    while (getline(iFile, line)) {
      ...
      if (line.find("relation") != string::npos) {
        ...
        } else {  // found normal attribute line
          // oFile<<line<<endl;
        ...
        */
        // int * attrVal =
        // longHexToArrInterval(bytes,shift,longLine,min,arrLen); attrVal =
        // longHexToArrInterval_nged(bytes,shift,vectorOfFile[fileNum],min,arrLen);
        longHexToArrInterval_nged(attrVal, bytes, shift, vectorOfFile[fileNum],
                                  min, arrLen);
        fileNum++;
        // write output NEW~
        string buff = "";
...
string HS::fileNameToLongHex_old(string fileName) {
  if (fileName.length() <= 0) {
    cout << "Error  blank file name in fileNameToLongHex of " << fileName
         << endl;
    return "";
  }
...
int *HS::longHexToArrInterval(int bytes, int shift, string &longLine, long min,
                              long arrLen) {
...
int HS::longHexToArrInterval_nged(int *arrIn, int bytes, int shift,
...
int *HS::longHexToARFFValueHexNStrArrIdxMethod(int bytes, string longLine) {
  int *attrValueHex =
      new int[commonLib::powInt(256, bytes)];  // to keep all value.
...
std::string HS::longToHex(long l, int bytes) {
  ...
  stream << std::setfill('0') << std::setw(bytes * 2) << std::hex << l;
  return stream.str();
};

string HS::featureSelectionToFile(int bytes, int shift, int h_start,
                                  int h_doForNSegment) {
  // a1.config.byteLength=i;
  string fileOutPath = "./OutARFF/";
  long step = config.step;  // 256;
  long nTh_start =
      0;  // should be h_start*256^(bytes-1) /*and add limit to doForNSegment*/
  ...
  for (; nTh_start + step <= maxLen; nTh_start += step) {
    ...
    long step_ = step;
    if ((nTh_start) > (maxLen - step)) {  // better check around the limit,
                                          // maybe overflow and mad them broken.
      step_ = nTh_start + (step - maxLen);
    }
    updateARFF(fileOutPath + fileInName, fileOutPath + fileOutName, bytes,
               shift, nTh_start, step_);  // 0->256*256(FF)
    ...
    // for(int i=0;i<4;i++)
    {
      ...
      attrCollect[i].insert(attrCollect[i].end(), moreValue[i].begin(),
                            moreValue[i].end());
      if (attrCollect[i].size() > 20001) {
        std::sort(attrCollect[i].begin(), attrCollect[i].end());
        attrCollect[i].erase(attrCollect[i].begin() + 20000,
                             attrCollect[i].end());
      }
    }
  }
  ...
  for (int i = 0; i < 4; i++) {
    ...
    string buff = "";
    for (int idx = 0; idx < attrCollect[i].size(); idx++) {  // print all in
                                                             // vect
    ...
    ofstream oFile;
    string outName = fileOutPath +
                     fileOutName.substr(0, fileOutName.length() - 5) + "_m" +
                     patch::to_string(i + 1) + ".fsattr";
    oFile.open(outName.c_str());
    if (oFile.is_open()) {
...
std::vector<attrValue> HS::wekaFeatureSelection(int method, string filePath) {
  ...
  // std::string command = "java -cp /root/weka-3-7-13/weka.jar weka.Run
  // -no-scan ";
  switch (method) {
    case 1:
      command += "weka.attributeSelection.CorrelationAttributeEval ";
      break;
    case 2:
      command += "weka.attributeSelection.GainRatioAttributeEval ";
      break;
    case 3:
      command += "weka.attributeSelection.InfoGainAttributeEval -B ";
      break;
    case 4:
      command += "weka.attributeSelection.SymmetricalUncertAttributeEval ";
      break;
      // default:break;
  }
  // command += "-s \"weka.attributeSelection.Ranker -T -1.7976931348623157E308
  // -N -1\" ";
  command +=
      "-s \"weka.attributeSelection.Ranker -T -1.7976931348623157E308 -N "
      "20005\" ";
  command += "-i ";
  command += "\"" + filePath + "\"";
  ...
  while (std::getline(f, line)) {
    if (line.find("Ranked attributes") != std::string::npos) {
      std::getline(f, line);
      for (std::getline(f, line); line.length() > 0;
           std::getline(f, line))  // Keep reading until got a blank line. May
                                   // not works with result from windows(/r)
      {
...
string HS::fsattrToarff(string fileInName, int iAttrNum, int shift) {
  ...
  string fileOutName = fileInName.substr(0, fileInName.length() - 7) + "_" +
                       sAttrNum + ".arff";  // arff to go into some classifier

  ofstream oFile;
  ifstream iFile;
  oFile.open(fileOutName.c_str());
  iFile.open(fileInName.c_str());
  // std::vector<attrFreq> vSelected;//i choose the easier method now. Go!
  // string.
  ...
  if (oFile.is_open() && iFile.is_open()) {
    ...
    for (int i = 0; i < vectorOfFile.size(); i++)  // all files
    {
      ...
      if (longLine.length() < config.lengthExtractable) continue;
...
double HS::arffToWeka(string fileInName, int classifier) {
  ...
  // java weka.Run -no-scan weka.classifiers.lazy.KStar -B 10 -M a -t
  // "C:\cygwin64\home\DCMote\_copy_oldDoc\v_proj\SP2\FileBackup\ARFF2\hex\lastPack_swap128.arff"
  return result;
}
...
int HS::exportObjbyteToPcd()  // export all of them
{
  ...
  if (iFile.is_open()) {
    ...
    while (std::getline(iFile, fileNameObjbyte)) {
      ...
      string longLine = "";          // i++;
      if (iFileObjbyte.is_open()) {  // if(i>10)return 0;
        string line;
      ...
      if (oFile.is_open()) {
        ...
        cout << ("something wrong with " + fileOutPathName) << endl;
      }
      oFile.close();
    }
  }
...
int HS::readFromNgedArr(int *arr, string fileName, int ngram, long min,
                        long arrLen) {
  fileName += ".pcd.sep.nged";
  ifstream iFile;
  iFile.open(fileName.c_str());
...
int HS::readFromNgedArrBool(bool *arrBool, string fileName, int ngram, long min,
                            long arrLen) {
  fileName += ".pcd.sep.nged";
  ifstream iFile;
  iFile.open(fileName.c_str());
...
int HS::readFromNgedsArr(int *arr, string fileName, int ngram, long min,
                         long arrLen, long &fileLinePointing) {
  ...
  if (iFile.is_open()) {
    ...
    while (getline(iFile, line)) {  // reading and parsing
      ...
      if (tabPos != string::npos && tabPos == (ngram * 2)) {
        ...
        if (lKey >= (min + arrLen) ||
            tabPos > (ngram * 2)) {  // time to jump out of this file
          break;
        }
      }
      fileLinePointing++;
...
std::vector<attrValue> HS::IGBinaryEval(int bytes, long inStart, long batchSize,
                                        double minThreshAdd, bool **arr,
                                        double *arrH) {
...
int HS::IGBinaryToFile(int bytes, long inStart, long inEnd, long batchSize) {
  ...
  for (long startTheBatch = inStart; startTheBatch < inEnd;
       startTheBatch += batchSize) {
  ...
  std::sort(attrCollect.begin(), attrCollect.end());
  ofstream oFile;
  string sFileName = "OutARFF/igbinary_" + patch::to_string(bytes) + "_" +
                     patch::to_string(inStart) + "-" + patch::to_string(inEnd) +
                     ".fsarff";
...
std::vector<attrValue> HS::TfidfEval(int bytes, long inStart, long batchSize,
                                     double minThreshAdd, int **arr,
                                     double *arrH, long *FileLinePointing,
                                     int *docLen) {
  int dataSetSize = vectorOfFile.size();
  long lStart = inStart;
  for (int i = 0; i < dataSetSize; i++) {
...
int HS::TfidfToFile(int bytes, long inStart, long inEnd, long batchSize) {
  ...
  for (long startTheBatch = inStart; startTheBatch < inEnd;
    ...
    cout << " AT " << commonLib::exec("date") << endl;
    std::vector<attrValue> moreValue =
        TfidfEval(bytes, startTheBatch, batchSize, minThresh, arr, arrH,
                  FileLinePointing, docLength);
  ...
  std::sort(attrCollect.begin(), attrCollect.end());
  ofstream oFile;
  string sFileName = "OutARFF/tfidf_" + patch::to_string(bytes) + "_" +
                     patch::to_string(inStart) + "-" + patch::to_string(inEnd) +
                     ".fsarff";
  oFile.open(sFileName.c_str());
  for (int i = 0; i < attrCollect.size(); i++) {
